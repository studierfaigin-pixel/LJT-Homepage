---
title: "On the Universal Truthfulness Hyperplane Inside LLMs"
collection: publications
category: conferences
permalink: /publication/2024-universal-truthfulness-hyperplane
date: 2024-11-01
venue: 'EMNLP 2024'
citation: 'Junteng Liu, Shiqi Chen, Yu Cheng, Junxian He. (2024). &quot;On the Universal Truthfulness Hyperplane Inside LLMs.&quot; <i>EMNLP 2024</i>.'
---
**Authors:** **Junteng Liu** (First author), Shiqi Chen, Yu Cheng, Junxian He

**Published in:** *EMNLP 2024* (Conference on Empirical Methods in Natural Language Processing)

**Note:** First author. Has GitHub code repository: Universal_Truthfulness_Hyperplane.
